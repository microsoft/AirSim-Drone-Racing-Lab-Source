...
bool PolynomialOptimizationNonLinear<_N>::setupFromVertices(
  ...
  // nlopt_ = std::make_shared<nlopt::opt>(nlopt::opt(optimization_parameters_.algorithm, n_optimization_parameters));
...
int PolynomialOptimizationNonLinear<_N>::optimizeTimeMellingerOuterLoop() {
  ...
  try {
    ...
    // std::cout << "error while setting up nlopt: " << std::endl;
    std::cout << "failed to set lower bound on segment_times(). returning nlopt::FAILURE(-1)" << std::endl;
    // LOG(ERROR) << "error while setting up nlopt: " << e.what() << std::endl;
    return nlopt::FAILURE;
  }
  ...
  try {
    ...
    std::cout << "error while optimizing with nlopt: This likely means the optimization aborted early." << std::endl;
    // LOG(ERROR) << "error while running nlopt. This likely means the optimization aborted early." << e.what()
