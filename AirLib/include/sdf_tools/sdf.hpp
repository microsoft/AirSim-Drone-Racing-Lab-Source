...
public:
                                                        ...
                                                        const bool enable_edge_gradients = false) const
  {
    ...
    for (int64_t x_idx{0l}; x_idx < GetNumXCells(); ++x_idx)
    {
      for (int64_t y_idx{0l}; y_idx < GetNumYCells(); ++y_idx)
      {
        for (int64_t z_idx{0l}; z_idx < GetNumZCells(); ++z_idx)
        {
...
public:
  ...
  virtual uint64_t SerializeSelf(
      ...
      = sdf_utils::SerializeFixedSizePOD<float>) const;
