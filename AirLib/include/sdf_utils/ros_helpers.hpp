...
namespace ROSHelpers
{
    ...
    inline Maybe::Maybe<T> GetParamOptional(const ros::NodeHandle& nh, const std::string& param_name, const std::string& calling_fn_name)
    {
        ...
        {
            ROS_DEBUG_STREAM_NAMED("params", "Cannot find " << nh.getNamespace() << "/" << param_name << " on parameter server for " << calling_fn_name);
            return Maybe::Maybe<T>();
        }
    }

    template <typename T>
    inline T GetParamRequired(const ros::NodeHandle& nh, const std::string& param_name, const std::string& calling_fn_name)
    {
        ...
        else
        {
            ...
            msg << "Cannot find " << nh.getNamespace() << "/" << param_name << " on parameter server for " << calling_fn_name << ": Value must be on paramter sever";
    ...
    template <typename T>
    inline T GetParamRequiredDebugLog(const ros::NodeHandle& nh, const std::string& param_name, const std::string& calling_fn_name)
    {
        ROS_DEBUG_STREAM_NAMED("params", "No default value for " << param_name << ": Value must be on paramter sever");
        T param_val;
        if (nh.getParam(param_name, param_val))
    ...
    template <typename T>
    inline std::vector<T> GetVectorRequired(const ros::NodeHandle& nh, const std::string& param_name, const std::string& calling_fn_name)
    {
        ROS_DEBUG_STREAM_NAMED("params", "No default value for " << param_name << ": Value must be on paramter sever");
        std::vector<T> vec;
        if (nh.getParam(param_name, vec))
    ...
    template <typename T>
    inline std::vector<T> GetVectorRequiredDebugLog(const ros::NodeHandle& nh, const std::string& param_name, const std::string& calling_fn_name)
    {
        ROS_DEBUG_STREAM_NAMED("params", "No default value for " << param_name << ": Value must be on paramter sever");
        std::vector<T> vec;
        if (nh.getParam(param_name, vec))
