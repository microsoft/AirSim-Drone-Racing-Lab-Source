...
#include <sdf_utils/math_helpers.hpp>
#include <sdf_utils/pretty_print.hpp>
#include <sdf_utils/simple_robot_model_interface.hpp>
...
namespace simple_linked_robot_model
{
    ...
    public:
        ...
        RobotJoint(const Eigen::Isometry3d& joint_transform,
                   const Eigen::Vector3d& joint_axis,
                   const int64_t parent_link_index,
                   const int64_t child_link_index,
                   const SimpleJointModel& joint_model,
    ...
    class SimpleLinkedRobot : public simple_robot_model_interface::SimpleRobotModelInterface<SimpleLinkedConfiguration, SimpleLinkedConfigAlloc>
    {
    protected:
        ...
        inline void UpdateTransforms()
        {
            ...
            for (size_t idx = 0; idx < joints_.size(); idx++)
            {
                ...
                {
                    const Eigen::Isometry3d joint_transform = Eigen::Translation3d(0.0, 0.0, 0.0) * Eigen::Quaterniond(Eigen::AngleAxisd(current_joint.JointModel().GetValue(), current_joint.JointAxis()));
                ...
                {
                    const Eigen::Translation3d joint_translation = (Eigen::Translation3d)(current_joint.JointAxis() * current_joint.JointModel().GetValue());
    ...
    public:
        ...
        static inline std::pair<std::vector<KinematicSkeletonElement>, bool> SanityCheckRobotModel(const std::vector<RobotLink>& links, const std::vector<RobotJoint>& joints)
        {
        ...
        SimpleLinkedRobot(const Eigen::Isometry3d& base_transform,
                          const std::vector<RobotLink>& links,
                          const std::vector<RobotJoint>& joints,
                          const SimpleLinkedConfiguration& initial_position,
        ...
        virtual simple_robot_model_interface::SimpleRobotModelInterface<SimpleLinkedConfiguration, SimpleLinkedConfigAlloc>* Clone() const
        {
        ...
        inline Eigen::Matrix<double, 6, Eigen::Dynamic> ComputeLinkPointJacobian(const std::string& link_name, const Eigen::Vector4d& link_relative_point) const
        {
        ...
        {
            const Eigen::Matrix<double, 6, Eigen::Dynamic> full_jacobian = ComputeLinkPointJacobian(link_name, link_relative_point);
