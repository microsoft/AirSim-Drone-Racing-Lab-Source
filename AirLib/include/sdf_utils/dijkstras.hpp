...
#include <sdf_utils/arc_helpers.hpp>
...
namespace arc_dijkstras
{
    ...
    public:
        ...
        uint64_t SerializeSelf(std::vector<uint8_t>& buffer) const
        {
            ...
            sdf_utils::SerializeFixedSizePOD<int64_t>(from_index_, buffer);
            sdf_utils::SerializeFixedSizePOD<int64_t>(to_index_, buffer);
            sdf_utils::SerializeFixedSizePOD<double>(weight_, buffer);
            sdf_utils::SerializeFixedSizePOD<EDGE_VALIDITY>(edge_validity_, buffer);
        ...
        uint64_t DeserializeSelf(const std::vector<uint8_t>& buffer, const uint64_t current)
        {
            ...
            const std::pair<int64_t, uint64_t> deserialized_from_index = sdf_utils::DeserializeFixedSizePOD<int64_t>(buffer, current_position);
            from_index_ = deserialized_from_index.first;
            current_position += deserialized_from_index.second;
            const std::pair<int64_t, uint64_t> deserialized_to_index = sdf_utils::DeserializeFixedSizePOD<int64_t>(buffer, current_position);
            to_index_ = deserialized_to_index.first;
            current_position += deserialized_to_index.second;
            const std::pair<double, uint64_t> deserialized_weight = sdf_utils::DeserializeFixedSizePOD<double>(buffer, current_position);
            weight_ = deserialized_weight.first;
            current_position += deserialized_weight.second;
            const std::pair<EDGE_VALIDITY, uint64_t> deserialized_validity = sdf_utils::DeserializeFixedSizePOD<EDGE_VALIDITY>(buffer, current_position);
    ...
    public:
                ...
                const std::function<uint64_t(const NodeValueType&, std::vector<uint8_t>&)>& value_serializer) const
        {
            ...
            sdf_utils::SerializeFixedSizePOD<double>(distance_, buffer);
            // Serialize the in edges
            sdf_utils::SerializeVector<GraphEdge>(in_edges_, buffer, GraphEdge::Serialize);
            // Serialize the in edges
            sdf_utils::SerializeVector<GraphEdge>(out_edges_, buffer, GraphEdge::Serialize);
            // Figure out how many bytes were written
            const uint64_t end_buffer_size = buffer.size();
            const uint64_t bytes_written = end_buffer_size - start_buffer_size;
            return bytes_written;
                ...
                const std::function<std::pair<NodeValueType, uint64_t>(const std::vector<uint8_t>&, const uint64_t)>& value_deserializer)
        {
            ...
            const std::pair<double, uint64_t> distance_deserialized = sdf_utils::DeserializeFixedSizePOD<double>(buffer, current_position);
            distance_ = distance_deserialized.first;
            current_position += distance_deserialized.second;
            // Deserialize the in edges
            const std::pair<std::vector<GraphEdge>, uint64_t> in_edges_deserialized = sdf_utils::DeserializeVector<GraphEdge>(buffer, current_position, GraphEdge::Deserialize);
            in_edges_ = in_edges_deserialized.first;
            current_position += in_edges_deserialized.second;
            // Deserialize the out edges
            const std::pair<std::vector<GraphEdge>, uint64_t> out_edges_deserialized = sdf_utils::DeserializeVector<GraphEdge>(buffer, current_position, GraphEdge::Deserialize);
    ...
    public:
                ...
                const std::function<uint64_t(const NodeValueType&, std::vector<uint8_t>&)>& value_serializer) const
        {
            ...
            sdf_utils::SerializeVector<GraphNode<NodeValueType, Allocator>>(nodes_, buffer, graph_state_serializer);
            // Figure out how many bytes were written
            const uint64_t end_buffer_size = buffer.size();
            const uint64_t bytes_written = end_buffer_size - start_buffer_size;
            return bytes_written;
                ...
                const std::function<std::pair<NodeValueType, uint64_t>(const std::vector<uint8_t>&, const uint64_t)>& value_deserializer)
        {
            ...
            const auto deserialized_nodes = sdf_utils::DeserializeVector<GraphNode<NodeValueType, Allocator>>(buffer, current, graph_state_deserializer);
    ...
    public:
                ...
                const std::function<uint64_t(const NodeValueType&, std::vector<uint8_t>&)>& value_serializer)
        {
            ...
            const auto index_serializer = std::bind(sdf_utils::SerializeFixedSizePOD<int64_t>, std::placeholders::_1, std::placeholders::_2);
            SerializeVector(result.second.first, index_serializer);
            // Serialze the distances
            const auto distance_serializer = std::bind(sdf_utils::SerializeFixedSizePOD<double>, std::placeholders::_1, std::placeholders::_2);
                ...
                const std::function<std::pair<NodeValueType, uint64_t>(const std::vector<uint8_t>&, const uint64_t)>& value_deserializer)
        {
            ...
            const auto index_deserializer = std::bind(sdf_utils::DeserializeFixedSizePOD<int64_t>, std::placeholders::_1, std::placeholders::_2);
            const auto prev_index_deserialized = sdf_utils::DeserializeVector<int64_t>(buffer, current_position, index_deserializer);
            ...
            const auto distance_deserializer = std::bind(sdf_utils::DeserializeFixedSizePOD<double>, std::placeholders::_1, std::placeholders::_2);
            const auto distance_deserialized = sdf_utils::DeserializeVector<double>(buffer, current_position, distance_deserializer);
